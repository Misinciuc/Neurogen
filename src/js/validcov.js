...
document.addEventListener("DOMContentLoaded", function () {
  const formCov = document.getElementById("form-cov");
  formCov.addEventListener("submit", SendForm);

  async function SendForm(e) {
    e.preventDefault();

    let error = formValidate(formCov);
    if (error === 0) {
    ...
    let formData = new FormData(formCov);
  }
  function formValidate(formCov) {
    let error = 0;
